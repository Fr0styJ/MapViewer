...
ICON_PATH = "icon.png"  # Make sure this file exists in the same folder
...
pygame.display.set_caption("DnD Map Viewer")
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_s:  # Save fog layer with 'S' key
            ...
            elif event.key == pygame.K_n:  # Create a new floor with 'N' key
                root = Tk()
                root.withdraw()  # Hide the main Tkinter window
                ...
                floors[current_floor]['fog'].fill((0, 0, 0, 255))
            elif event.key == pygame.K_F1:  # Toggle help menu
    ...
    # Display help menu if toggled
    if show_help:
        font = pygame.font.Font(None, 24)
        help_text = [
            "Controls:",
            "Left Click - Reveal fog",
            "Right Click - Restore fog",
            "Middle Click - Pan",
        ...
        help_surface.fill((0, 0, 0, 200))  # Semi-transparent background
        for i, line in enumerate(help_text):
            text_surf = font.render(line, True, (255, 255, 255))
            help_surface.blit(text_surf, (10, 10 + i * 20))
        screen.blit(help_surface, (20, 20))  # Draw help menu in top-left
